...
export async function POST(req:Request){try{
 ...
 const db=database(),r=await db.prepare("SELECT * FROM products WHERE slug=? AND status='published'").bind(d.slug).first();if(!r)throw new ApiError("This guide is unavailable.",404);const p=productFromRow(r),f=p.commerce!.funnel;if(!f.enabled||f.kind!=='free_guide'||(await planFor(String(r.owner))).tier!=='pro')throw new ApiError("This free guide funnel is unavailable.",404);
