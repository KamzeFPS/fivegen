...
export async function POST(req:Request){try{
  ...
  await database().prepare("INSERT INTO checkout_intents (id,owner,product_id,account,amount,mode,items,quantity,platform_fee,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)")
    .bind(id,row.owner,row.id,String(row.stripe_account||""),quote.total,mode,JSON.stringify(quote.items),quote.quantity,platformFee,Date.now()).run();
  ...
  else if(platformFee>0)fields.set("payment_intent_data[application_fee_amount]",String(platformFee));
