...
export default async function Success({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{session_id?:string;token?:string}>}){
  ...
  return <div className="storefront"><nav className="store-nav"><a href="/"><Brand/></a></nav><main className="delivery-card panel"><span className="success-symbol"><Check size={30}/></span><h1>{error?"Let’s check your access.":"Something good is yours."}</h1><p>{error||`Your ${title} package is ready. Save this private access link for your downloads${subscription?" and subscription management":""}.`}</p>{!error&&<div className="delivery-files">{items.map(item=><a key={item.id} className="button primary" href={`/api/download/${item.slug}?token=${encodeURIComponent(token)}`}><ArrowDownToLine size={17}/>{item.title}</a>)}</div>}{subscription&&token&&<ManageSubscription token={token}/>}<a className="text-link" href={`/p/${slug}`}>Back to product</a></main></div>;
}
