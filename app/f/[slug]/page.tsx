...
export default async function Funnel({params}:{params:Promise<{slug:string}>}){
  ...
  return <div className="published-funnel"><nav className="store-nav"><a href="/"><Brand/></a><span>{String(row.seller_name||"Independent creator")}</span></nav><FunnelView funnel={c.funnel} checkout={<Purchase slug={slug} price={p.price} available={p.price===0||!!(row.stripe_account&&binding("STRIPE_SECRET_KEY"))} billing={c.billing} hasCode={!!c.deal.code} volume={c.deal.type==="volume"} initialQuote={quote} cta={c.funnel.cta} endsAt={c.deal.endsAt}/>}/><footer className="store-footer"><a href={`/p/${slug}`}>Product details</a><a href="/">Made with FiveGen</a></footer></div>;
}
