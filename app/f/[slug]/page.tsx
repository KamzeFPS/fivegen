...
import { binding,database,productFromRow,stripeConfigured } from "@/lib/server";
...
export default async function Funnel({params}:{params:Promise<{slug:string}>}){
  ...
  const p=productFromRow(row),c=p.commerce!;if(!c.funnel.enabled)notFound();
  const quote=await quoteProduct(slug).then(q=>q.quote).catch(()=>null);
  return <div className="published-funnel"><nav className="store-nav"><a href="/"><Brand/></a><span>{String(row.seller_name||"Independent creator")}</span></nav><FunnelView funnel={c.funnel} checkout={c.funnel.kind==="free_guide"?<LeadForm slug={slug} cta={c.funnel.cta}/>:<Purchase slug={slug} price={p.price} available={p.price===0||!!(row.stripe_account&&stripeConfigured()&&binding("STRIPE_WEBHOOK_SECRET"))} billing={c.billing} hasCode={!!c.deal.code} volume={c.deal.type==="volume"} initialQuote={quote} cta={c.funnel.cta} endsAt={c.deal.endsAt}/>}/><footer className="store-footer"><a href={`/p/${slug}`}>Product details</a><a href="/">Made with FiveGen</a></footer></div>;
}
