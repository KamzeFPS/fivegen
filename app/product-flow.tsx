...
export function CreateProductFlow({ textCost=10, open, onOpenChange, brief, onBriefChange, step, onStepChange, busy, aiReady, signedIn, error, onCreate, onConnectAI }: {
    ...
    <DialogContent className="flow-dialog" showCloseButton={false}>
      ...
      <div className="flow-body" key={step}>
        {step === 0 ? <div className="format-options flow-formats">{formats.map((format, index) => {
          ...
          <div className={`creation-mode ${aiReady ? "ready" : ""}`}><Sparkles size={20} /><div><strong>{aiReady ? "FiveGen AI is ready" : "Start with an editable draft"}</strong><p>{aiReady ? `Content uses ${textCost} credits per step (up to ${textCost*12} for a complete product). Images and videos use separate credits in Marketing.` : "The administrator is configuring FiveGen AI. You can create and edit a starter now."}</p>{!aiReady && <button type="button" className="text-link" onClick={onConnectAI}>View AI & credits <ArrowUpRight size={14} /></button>}</div></div>
