...
import { ArrowLeft, ArrowRight, ArrowUpRight, BookOpen, Check, ChevronDown, FileText, Globe, Layers3, Loader2, Package, CalendarDays, Users, Sparkles, TrendingUp, X, Zap } from "lucide-react";
...
import { formats, money, publishContentError, type Brief, type Product } from "@/lib/product";
...
export function CreateProductFlow({ remainingProducts, textCost=10, open, onOpenChange, brief, onBriefChange, step, onStepChange, busy, aiReady, signedIn, error, generationMode, onModeChange, onCreate, onConnectAI }: {
  remainingProducts?:number; textCost?:number; open: boolean; onOpenChange: (open: boolean) => void; brief: Brief; onBriefChange: (brief: Brief) => void;
  ...
  generationMode: "auto" | "manual"; onModeChange: (mode: "auto" | "manual") => void;
}) {
    ...
    <DialogContent className="flow-dialog" showCloseButton={false}>
      ...
      <div className="flow-body" key={step}>
        {step === 0 ? <div className="format-options flow-formats">{formats.map((format, index) => {
          const Icon = [BookOpen, Zap, Layers3, TrendingUp, FileText, Package,CalendarDays,Users][index];
          return <button type="button" key={format} className={brief.format === format ? "chosen" : ""} aria-pressed={brief.format === format} onClick={() => update({ format })}><Icon size={22} /><span><strong>{recipeFor(format).label}</strong><small>{recipeFor(format).description}</small></span><span className="radio-dot">{brief.format === format && <span />}</span></button>;
          ...
          <label>{recipeFor(brief.format).outcome}<textarea required minLength={12} maxLength={12000} placeholder={recipeFor(brief.format).example} value={brief.description} onChange={(event) => update({ description: event.target.value })} /></label>
          ...
          <label>What makes your approach different? <span className="optional">Optional</span><textarea rows={2} maxLength={2000} value={brief.angle||""} onChange={e=>update({angle:e.target.value})} placeholder="Your method, lived experience, niche, constraints, or examples you want included."/></label>{brief.format==="Challenge"&&<label>Challenge length · days<input type="number" min={3} max={30} value={brief.duration||7} onChange={e=>update({duration:Number(e.target.value)})}/></label>}<div className="deliverable-chips">{recipeFor(brief.format).deliverables.map(v=><span key={v}><Check size={13}/>{v}</span>)}</div><p className="flow-hint">Next: price, creative direction, and generation options.</p>
          ...
          <label>Create with<Select value={generationMode} onValueChange={(value) => onModeChange(value as "auto" | "manual")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="auto">FiveGen AI</SelectItem><SelectItem value="manual">Write it myself · no AI credits</SelectItem></SelectContent></Select></label>
          <div className={`creation-mode ${aiReady && generationMode === "auto" ? "ready" : ""}`}><Sparkles size={20} /><div><strong>{generationMode === "manual" ? "A blank draft, ready for your content" : !signedIn ? "Sign in to use FiveGen AI" : aiReady ? "FiveGen AI is ready" : "AI generation is currently unavailable"}</strong><p>{generationMode === "manual" ? `Start with an empty ${recipeFor(brief.format).unit.toLowerCase()}. Write your content or add it from your connected assistant before publishing. Manual drafts are unlimited and use no AI credits.` : aiReady ? `${remainingProducts===undefined?"Your first 3 AI products each month are included.":`${remainingProducts} of your 3 included AI products remain this month.`} After that, content uses ${textCost} credits per step (up to ${textCost*(brief.format==="Challenge"?(brief.duration||7)+2:32)} per product). You’ll confirm credit spending before generation. Images and videos always need credits.` : "Check service availability in AI & credits, or choose to write the product yourself."}</p>{signedIn && !aiReady && generationMode === "auto" && <button type="button" className="text-link" onClick={onConnectAI}>View AI & credits <ArrowUpRight size={14} /></button>}</div></div>
          {error && <p role="alert" className="error-banner">{error}</p>}
        </form>}
      </div>
      <footer className="flow-footer"><button type="button" className="button secondary" disabled={busy} onClick={() => step === 0 ? onOpenChange(false) : onStepChange(step - 1)}>{step > 0 && <ArrowLeft size={16} />}{step === 0 ? "Cancel" : "Back"}</button><span className="flow-footer-note">Step {step + 1} of 3</span>{step === 0 ? <button key="choose-format" type="button" className="button primary" onClick={() => onStepChange(1)}>Continue <ArrowRight size={16} /></button> : <button key={`step-submit-${step}`} type="submit" form={step === 1 ? "fivegen-brief" : "fivegen-create"} className="button primary" disabled={busy || (step === 2 && signedIn && generationMode === "auto" && !aiReady)}>{busy ? <Loader2 size={16} className="spin" /> : step === 2 ? <Sparkles size={16} /> : null}{busy ? "Creating…" : step === 1 ? "Review & create" : !signedIn ? "Sign in & create" : generationMode === "auto" ? "Generate product" : "Create blank draft"}{step === 1 && <ArrowRight size={16} />}</button>}</footer>
    </DialogContent>
  </Dialog>;
}
...
export function PublishReview({ product, open, onOpenChange, busy, paymentsReady, onPayments, onPublish }: { product: Product; open: boolean; onOpenChange: (open: boolean) => void; busy: boolean; paymentsReady: boolean; onPayments: () => void; onPublish: () => void }) {
  const billing = product.commerce?.billing || "once";
  const canSell = product.price === 0 || paymentsReady ;
  const contentError = publishContentError(product.content);
