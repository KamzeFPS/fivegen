...
export function CreateProductFlow({ textCost=10, open, onOpenChange, brief, onBriefChange, step, onStepChange, busy, aiReady, signedIn, error, generationMode, onModeChange, onCreate, onConnectAI }: {
  textCost?:number; open: boolean; onOpenChange: (open: boolean) => void; brief: Brief; onBriefChange: (brief: Brief) => void;
    ...
    <DialogContent className="flow-dialog" showCloseButton={false}>
      ...
      <div className="flow-body" key={step}>
        {step === 0 ? <div className="format-options flow-formats">{formats.map((format, index) => {
          ...
          return <button type="button" key={format} className={brief.format === format ? "chosen" : ""} aria-pressed={brief.format === format} onClick={() => update({ format })}><Icon size={22} /><span><strong>{recipeFor(format).label}{["Coaching session","Community"].includes(format)&&<span className="format-pro-label">PRO DELIVERY</span>}</strong><small>{recipeFor(format).description}</small></span><span className="radio-dot">{brief.format === format && <span />}</span></button>;
          ...
          <div className={`creation-mode ${aiReady && generationMode === "auto" ? "ready" : ""}`}><Sparkles size={20} /><div><strong>{generationMode === "manual" ? "A blank draft, ready for your content" : !signedIn ? "Sign in to use FiveGen AI" : aiReady ? "FiveGen AI is ready" : "AI generation is currently unavailable"}</strong><p>{generationMode === "manual" ? `Start with an empty ${recipeFor(brief.format).unit.toLowerCase()}. Write your content or add it from your connected assistant before publishing. This uses one product slot and no AI credits.` : aiReady ? `Content uses ${textCost} credits per step (up to ${textCost*(brief.format==="Challenge"?(brief.duration||7)+2:12)} for a complete product). Images and videos use separate credits in Marketing.` : "Check service availability in AI & credits, or choose to write the product yourself."}</p>{signedIn && !aiReady && generationMode === "auto" && <button type="button" className="text-link" onClick={onConnectAI}>View AI & credits <ArrowUpRight size={14} /></button>}</div></div>
