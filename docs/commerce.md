...
FiveGen Free supports 10 products, including drafts, standard product pages, exports, basic Stripe or Whop selling, and the revenue dashboard. AI usage uses the creator's configured providers. FiveGen Pro costs USD 29/month or USD 290/year and supports 100 products plus deals, opt-in upsells, funnel pages, and recurring product prices. FiveGen adds no transaction fee; provider fees remain separate.
...
- Whop links use the pricing, offers, subscriptions, and delivery configured in Whop. FiveGen's offer calculation is used by its Stripe checkout and free offer delivery.
...
2. Add a connected-account webhook destination at `/api/webhooks/stripe` for `checkout.session.completed`, `checkout.session.async_payment_succeeded`, and `invoice.paid`. Store its signing secret as `STRIPE_WEBHOOK_SECRET`.
3. Add a platform-account webhook destination at the same URL for `checkout.session.completed`, `customer.subscription.created`, `customer.subscription.updated`, and `customer.subscription.deleted`. Store this signing secret separately as `STRIPE_BILLING_WEBHOOK_SECRET`.
