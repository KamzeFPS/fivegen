...
let owner='planner-owner',calls=0,fail=false,reserved=0,returned=0,hold=null;
const deps={z,zodToJsonSchema,ApiError,database:()=>storage.DB,identity:async()=>({userId:owner}),sameOrigin:()=>{},failure:e=>Response.json({error:e.message},{status:e.status||400}),textCredits,
 ...
 textGeneration:async(_owner,_system,input)=>{calls++;if(hold)await hold;if(fail)throw new ApiError('Provider temporarily unavailable',503);const prompt=JSON.parse(input).message;return {answer:'I suggest a focused, practical product tailored to your intended audience.',questions:['What experience should the reader already have?'],plan:{title:prompt.slice(0,95),description:'A complete, practical product with concrete exercises and reusable resources.',audience:'New independent illustrators',format:'Guide',language:'English',duration:7,deliverables:[{name:'Detailed guide',detail:'Original chapters with useful real examples.'},{name:'Working checklist',detail:'A complete editable checklist in Markdown.'},{name:'Email template',detail:'A fully written editable client email.'}],direction:'A clear, practical handoff system tailored to independent illustrators.',assumptions:['No external integrations required.']}};},
};
...
try{
 ...
 response=await post({...paid,requestId:randomUUID(),maxCredits:10});assert.equal(response.status,200);const second=await response.json();assert.equal(second.spent,10);assert.equal(reserved,10);
