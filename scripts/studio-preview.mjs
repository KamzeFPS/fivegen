...
const termsVersion=readFileSync('lib/terms.ts','utf8').match(/TERMS_VERSION='([^']+)'/)?.[1];
assert.ok(termsVersion,'Current terms version must be available for the isolated QA identity');
storage.sqlite.prepare('INSERT INTO terms_acceptances VALUES (?,?,?,?,?)').run(randomUUID(),owner,termsVersion,'studio-qa@example.test',Date.now());
storage.close();
