...
try{
  let r=await fetch(base+'/mcp',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});assert.equal(r.status,401);assert.ok(r.headers.get('www-authenticate').includes('oauth-protected-resource'));
  ...
  const tokenBody=new URLSearchParams({grant_type:'authorization_code',client_id:clientId,resource:base+'/mcp',redirect_uri:query.get('redirect_uri'),code:callback.searchParams.get('code'),code_verifier:verifier});
  ...
  client=new Client({name:'fivegen-integration-test',version:'1.0.0'});await client.connect(new StreamableHTTPClientTransport(new URL(base+'/mcp'),{requestInit:{headers:{Authorization:'Bearer '+credentials.access_token}}}));
  ...
  assert.equal((await fetch(base+'/mcp',{method:'POST',headers:{Authorization:'Bearer '+credentials.access_token,origin:'https://evil.example','Content-Type':'application/json'},body:'{}'})).status,403);
  const refreshBody=new URLSearchParams({grant_type:'refresh_token',client_id:clientId,resource:base+'/mcp',refresh_token:credentials.refresh_token});const fresh=await token(refreshBody);assert.equal(fresh.r.status,200);assert.equal((await token(refreshBody)).r.status,400);
  assert.equal((await fetch(base+'/mcp',{method:'POST',headers:{Authorization:'Bearer '+credentials.access_token,'Content-Type':'application/json'},body:'{}'})).status,401);
  const revoke=await json('/api/connections','DELETE',{id:connectionId},{cookie,origin:base});assert.equal(revoke.r.status,200);
  assert.equal((await fetch(base+'/mcp',{method:'POST',headers:{Authorization:'Bearer '+fresh.d.access_token,'Content-Type':'application/json'},body:'{}'})).status,401);
